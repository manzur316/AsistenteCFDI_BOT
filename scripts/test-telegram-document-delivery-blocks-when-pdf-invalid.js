...
const tempRoot = path.join(root, "runtime", "test-telegram-delivery-blocks-invalid-pdf");
...
async function main() {
  ...
  const result = await runSandboxDocumentDeliverySend({
    draft: {
      draft_id: "DRAFT-TELEGRAM-BLOCK-PDF",
      client_id: "CLI-REAL-BILBAO",
      invoice_status: "SANDBOX_TIMBRADO",
      current_client: { display_name: "Real Bilbao" },
      sandbox_pac_summary: {
  ...
  console.log("Telegram Document Delivery Blocks Invalid PDF Tests");
  try {
    ...
    console.log(" - telegram_delivery_blocks_invalid_pdf: PASS");
    console.log("\nPASS total: 1/1");
  } catch (error) {
    console.log(` - telegram_delivery_blocks_invalid_pdf: FAIL (${error.message})`);
    console.log("\nPASS total: 0/1");
    process.exit(1);
  }
}
