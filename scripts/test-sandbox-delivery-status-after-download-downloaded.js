...
const tempRoot = path.join(root, "runtime", "test-sandbox-delivery-status-after-download-downloaded");
...
fs.writeFileSync(xmlPath, "<?xml version=\"1.0\"?><cfdi:Comprobante xmlns:cfdi=\"http://www.sat.gob.mx/cfd/4\" Version=\"4.0\"><cfdi:Complemento><tfd:TimbreFiscalDigital xmlns:tfd=\"http://www.sat.gob.mx/TimbreFiscalDigital\" UUID=\"00000000-0000-4000-8000-000000000717\" /></cfdi:Complemento></cfdi:Comprobante>");
...
const result = runSandboxDocumentDeliveryStatus({
  draft: {
    draft_id: "DRAFT-DELIVERY-AFTER-DOWNLOAD-717F",
...
assert.strictEqual(result.output.persisted_artifact_status, "DOWNLOADED");
assert.strictEqual(result.output.artifact_status, "DOWNLOADED");
assert.strictEqual(result.output.artifact_status_inferred_from_documents, false);
...
console.log("Sandbox Delivery Status After Download Downloaded Tests");
console.log(" - downloaded_status_stays_downloaded_with_valid_documents: PASS (DOWNLOADED)");
console.log("\nPASS total: 1/1");
