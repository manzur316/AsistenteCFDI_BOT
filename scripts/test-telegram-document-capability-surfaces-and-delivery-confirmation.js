...
check("20_confirmar_envio_desde_post_download_valido", () => assert.strictEqual(executeCode(handleCode, callbackFor("DELIVERY_CONFIRM_PROVIDER_EMAIL", downloaded, { screen_id: "POST_DOWNLOAD_DELIVERY_READY", state: "POST_DOWNLOAD_DELIVERY_READY", token: "confirmpost001" })).should_execute_sandbox_action, true));
check("21_confirmar_envio_desde_invoice_detail_valido", () => assert.strictEqual(executeCode(handleCode, callbackFor("DELIVERY_CONFIRM_PROVIDER_EMAIL", downloaded, { source_module: "INVOICES", screen_id: "INVOICE_DETAIL", state: "INVOICE_DETAIL", token: "confirminv001" })).should_execute_sandbox_action, true));
check("22_confirmar_envio_desde_document_detail_valido", () => assert.strictEqual(executeCode(handleCode, callbackFor("DELIVERY_CONFIRM_PROVIDER_EMAIL", downloaded, { screen_id: "DOCUMENT_DETAIL", state: "DOCUMENT_DETAIL", token: "confirmdoc001" })).should_execute_sandbox_action, true));
