...
if (workflow) {
  ...
  checks.push({ name: "webhook_responds_on_received", pass: webhookNode.parameters.responseMode === "onReceived", value: webhookNode.parameters.responseMode });
  ...
  try {
    const result = executeCode(extractCode, makeWebhookInput({
      update_id: 4102,
      callback_query: { id: "cb-test", from: { id: "chat-local" }, data: "/pendientes" },
    }))[0].json;
