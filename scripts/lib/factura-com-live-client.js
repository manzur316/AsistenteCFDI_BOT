  ...
  if (typeof value === "object") {
    ...
    for (const [key, item] of Object.entries(value)) {
      if (/api[-_ ]?key|secret|plugin|token|authorization|password|f-api-key|f-secret-key|f-plugin/i.test(key)) {
        out[key] = "[REDACTED]";
      } else if (/^(rfc_shape|normalized_rfc_shape|normalized_rfc_length|normalized_rfc_lengths|rfc_has_hidden_characters|rfc_hidden_characters_detected|invalid_rfc_shape_detected|rfc_shape_values|client_rfc_validation)$/i.test(key)) {
