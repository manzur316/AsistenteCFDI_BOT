...
if (behavior.normal) {
  ...
  checks.push({ name: "normal_starts_invoice_wizard", pass: behavior.normal.action === "INVOICE_WIZARD" && String(behavior.normal.telegram_message).includes("Falta el monto"), value: `${behavior.normal.action}/${conceptId(behavior.normal)}` });
  ...
  checks.push({ name: "wizard_does_not_create_draft_sql", pass: !String(behavior.normal.persistence_sql).includes("INSERT INTO cfdi_drafts") && String(behavior.normal.persistence_sql).includes("INSERT INTO chat_states"), value: "preview first" });
  ...
  checks.push({ name: "restore_passthrough", pass: behavior.restored.action === "INVOICE_WIZARD" && behavior.restored.update_id === 501, value: `${behavior.restored.action}/${behavior.restored.update_id}` });
