...
Optional workflow-sync flags:

```text
--apply-workflow-sync=true|false
--confirm-workflow-sync=true
--activate-workflow-after-sync=true|false
```

Workflow sync does not apply by default and is blocked unless both `--apply-workflow-sync=true`
and `--confirm-workflow-sync=true` are provided, with `--safe=false` when applying.
...
workflow_not_found: no active workflow named cfdi_telegram_local_ingest
workflow_diff_detected
```
